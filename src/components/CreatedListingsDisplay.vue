<template>
  <hr><hr><hr>
  <div class="title"><h2 class="title">Created Outings</h2></div>
  <v-container fluid class="mx-auto">
    ...
    >
      ...
      <div v-else>
        <p class="text-center">You have no created outings for now. Browse through our outings at Explore Outings or at the Home Page and start saving!</p>
      </div>
    </v-infinite-scroll>
  </v-container>
</template>
...
methods: {
  ...
  async getcreated() {
    ...
    if (userDocSnap.exists()) {
      ...
      const outings = await this.fetchOutingsDetails(created);
      this.outings = shuffle(outings);
      // Check if user has no created outings
      if (created.length === 0) {
  ...
  async bookmarkListing(listing) {
    ...
    try {
      ...
      // Update tempIcon to reflect new state
      this.tempIcon = listing.bookmarked ? 'mdi-bookmark-outline' : 'mdi-bookmark';
    } catch (error) {
      console.error('Error updating document:', error);
    }
  },
  onMouseOver(listing) {
    this.hoveringListingID = listing.listingID;
    this.tempIcon = this.getHoverIcon(listing);
  },
  onMouseLeave() {
    this.hoveringListingID = null;
    this.tempIcon = null;
  },
  getIcon(listing) {
    if (this.tempIcon && this.hoveringListingID === listing.listingID) {
      return this.tempIcon;
    }
    return listing.bookmarked ? 'mdi-bookmark' : 'mdi-bookmark-outline';
  },
  getHoverIcon(listing) {
